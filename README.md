...
| `SENTINEL_LABEL_ENABLE`      | `true`                        | Enable label-based opt-in tracking                |
| `SENTINEL_WATCH_LABEL`       | `com.sentinel.watch.enable`   | Label key used to opt containers into tracking    |
| `SENTINEL_WATCH_LABEL_VALUE` | `true`                        | Required label value for opt-in                   |
...
## 🏷️ Label-Based Tracking (Docker Compose)

By default, Sentinel now runs with `SENTINEL_WATCH_ALL=false`.
That means only explicitly labeled containers are tracked.

Add this label to any service you want Sentinel to watch:
